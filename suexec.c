...
#define FRONTPAGE_MAGIC_DIR "/usr/local/frontpage/version5.0/apache-fp/_vti_bin"
...
static void log_close()
{
	if (log != NULL) {
		...
		if ((fcntl(fileno(log), F_SETFD, FD_CLOEXEC) == -1)) {
			fprintf(stderr, "error: can't set close-on-exec flag\n");
			perror("fcntl");
			exit(122);
		}
#else
...
static void clean_env(void)
{
	...
	if (site_root) {
		char *tmp;
		int root_len = strlen(site_root);
		for (idx = 0; idx < cidx; idx++) {
			if (0 != (tmp = strchr(cleanenv[idx], '=')) 
				&& strncmp(++tmp, site_root, root_len) == 0) {
				int offset = tmp - cleanenv[idx];
				if (0 != (tmp = strdup(cleanenv[idx]))) {
					cleanenv[idx] = tmp;
					strcpy(tmp + offset, tmp + offset + root_len);
				}
			}
		}
	}
...
int main(int argc, char *argv[])
{
	...
	if (0 != site_root) {
		...
		if (!strncmp(cwd, FRONTPAGE_MAGIC_DIR, strlen(FRONTPAGE_MAGIC_DIR))) {
#ifdef AP_SUEXEC_UMASK
		    /*
    ...
    if ((gid != dir_info.st_gid) ||
        ...
        log_err("program (%ld/%ld) mismatch "
                "with directory (%ld/%ld) or account (%ld)\n",                
